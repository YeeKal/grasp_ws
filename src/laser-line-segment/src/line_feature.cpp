...
least LineFeature::leastsquare(int start,int end,int firstfit)
{
	...
	if(firstfit == 1)
	{
		...
		for(k = start;k <= end;k++)
		{
			mid1+=range_data_.xs[point_num_[k]];
			mid2+=range_data_.ys[point_num_[k]];
			mid3+=range_data_.xs[point_num_[k]]*range_data_.xs[point_num_[k]];
			mid4+=range_data_.ys[point_num_[k]]*range_data_.ys[point_num_[k]];
			mid5+=range_data_.xs[point_num_[k]]*range_data_.ys[point_num_[k]];
		}
	}
	else
	{
		if(firstfit == 2)
		{
			mid1+=range_data_.xs[point_num_[end]];
			mid2+=range_data_.ys[point_num_[end]];
			mid3+=range_data_.xs[point_num_[end]]*range_data_.xs[point_num_[end]];
			mid4+=range_data_.ys[point_num_[end]]*range_data_.ys[point_num_[end]];
			mid5+=range_data_.xs[point_num_[end]]*range_data_.ys[point_num_[end]];
		}
		else
		{
			mid1+=range_data_.xs[point_num_[start]];
			mid2+=range_data_.ys[point_num_[start]];
			mid3+=range_data_.xs[point_num_[start]]*range_data_.xs[point_num_[start]];
			mid4+=range_data_.ys[point_num_[start]]*range_data_.ys[point_num_[start]];
			mid5+=range_data_.xs[point_num_[start]]*range_data_.ys[point_num_[start]];
		}
	}
	w1 = n*mid5-mid1*mid2;
...
bool LineFeature::detectline(const int start,const int num)
{
	...
	{
		//到直线的垂直距离
		error1 = fabs(((m_least.a)*range_data_.xs[point_num_[k]]+(m_least.b)*range_data_.ys[point_num_[k]]+m_least.c))/sqrt((1+(m_least.a)*(m_least.a)));
		...
		theta = params_.angle_increment*point_num_[k] + params_.angle_start;
		...
		//计算到预测点之间的误差
		error2 = distance_point(range_data_.xs[point_num_[k]],range_data_.ys[point_num_[k]],m_pn.x,m_pn.y);	
...
int LineFeature::detectfulline(const int start)
{
	...
	while(flag2)
	{		
		if((fabs(a*range_data_.xs[point_num_[n2]]+b*range_data_.ys[point_num_[n2]]+c)/(sqrt(1+a*a)))<params_.least_thresh)
		{
	...
	while(flag1)
	{
		if((fabs(a*range_data_.xs[point_num_[n1]]+b*range_data_.ys[point_num_[n1]]+c)/(sqrt(1+a*a)))<params_.least_thresh)
		{
	...
	n1 = n1+1;
	// n1 n2 refer to the number in point_num_
	m_temp.left = n1;
	m_temp.right = n2;
...
void LineFeature::cleanline()
{
	...
	for(p = 0; p < (m_line.size() - 1); p++)
	{
		...
		if(m >= n)
		{
			...
			{
				line_temp = m_iter;
				error1 = fabs(((m_line[p].a)*range_data_.xs[point_num_[m_iter]]+(m_line[p].b)*range_data_.ys[point_num_[m_iter]]+m_line[p].c))/sqrt((1+(m_line[p].a)*(m_line[p].a)));
				error2 = fabs(((m_line[q].a)*range_data_.xs[point_num_[m_iter]]+(m_line[q].b)*range_data_.ys[point_num_[m_iter]]+m_line[q].c))/sqrt((1+(m_line[q].a)*(m_line[q].a)));
				if(error1 > error2)
				{
					break;
				}
...
bool LineFeature::detect_short_line(const int n1,const int n2)
{
	
	if(distance_point(range_data_.xs[point_num_[n1]],range_data_.ys[point_num_[n1]],range_data_.xs[point_num_[n2]],range_data_.ys[point_num_[n2]])<params_.min_line_length)
	{
		return false;
	}
	else
...
void LineFeature::generate(std::vector<gline>& temp_line2)
{
	...
	{
		m = point_num_[m_line[i].left];
		n = point_num_[m_line[i].right];
...
void LineFeature::extractLines(std::vector<line>& temp_line1,std::vector<gline>& temp_line2)
{
	...
	for(unsigned int i = 0; i < (point_num_.size() - params_.min_line_points) ;i++)
	{
		...
		// i means the index in point_num_
	...
	{
		if(!detect_short_line(m_line[p].left,m_line[p].right))
		{
...
//extract lines with the given index
// add by yee-2019-11-22
void LineFeature::extractLinesWithIndex(std::vector<line>& temp_line1,std::vector<gline>& temp_line2,std::vector<unsigned int> &index)
{
	int line_include = 0;
	m_line.clear();
	point_num_ = index;
